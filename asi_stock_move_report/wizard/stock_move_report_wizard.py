...
class StockMoveReportWizard(models.TransientModel):
    ...
    @api.onchange('period_type', 'day_date', 'week_month', 'week_year', 'week_number', 'month', 'month_year', 'year', 'custom_date_from', 'custom_date_to')
    ...
    def action_generate_report(self):
        """
        Genera el reporte PDF de movimientos de inventario
        """
        self.ensure_one()
        ...
        return self.env.ref('asi_stock_move_report.action_report_stock_move').report_action(
            self, data=data
        )
